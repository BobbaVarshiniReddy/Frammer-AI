...
export default function UsageTrends() {
  ...
  useEffect(() => {
    ...
    const url = plotSelected.endpoint === "duration"
      ? `http://127.0.0.1:8000/trends/plot/duration/${plotSelected.value}`
      : `http://127.0.0.1:8000/trends/plot/${plotSelected.value}`;
    fetch(url)
      .then((res) => res.json())
      .then((resData) => {
  ...
  useEffect(() => {
    setKpiLoading(true);
    setKpiError(null);
    fetch(`http://127.0.0.1:8000/trends/kpi/${kpiSelected.value}`)
      .then((res) => res.json())
      .then((resData) => {
